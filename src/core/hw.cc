...
#ifdef JSONC
json_object* json_object_new_string_str(const string& s)
{
  return json_object_new_string_len(s.c_str(), s.length());
}

json_object* hwNode::asJSONObj(unsigned level)
{
  ...
  if(visible(getClassName()))
  {
    ...
    if (getSerial() != "")
      json_object_object_add(jobj, "serial", json_object_new_string_str(::enabled("output:sanitize") ? REMOVED : getSerial()));
  ...
  if(countChildren()>0)
  {
    if(visible(getClassName()))
    {
      json_object *jchild = json_object_new_array();

      for (unsigned int j = 0; j < countChildren(); j++)
        json_object_array_add(jchild, getChild(j)->asJSONObj(level + 1));

      json_object_object_add(jobj, "children", jchild);
    }
    else
    {
      for (unsigned int j = 0; j < countChildren(); j++)
        json_object_array_add(jobj, getChild(j)->asJSONObj(level + 1));
    }
  }
...
{
  json_object *jobj = asJSONObj(level);
  if (!jobj) return "";

#ifdef JSON_C_TO_STRING_NOSLASHESCAPE
  string ret (json_object_to_json_string_ext(jobj, JSON_C_TO_STRING_PRETTY | JSON_C_TO_STRING_NOSLASHESCAPE));
#else
  string ret (json_object_to_json_string_ext(jobj, JSON_C_TO_STRING_PRETTY));
#endif
